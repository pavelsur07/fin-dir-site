...
Счётчик `112100592` вставлен перед `</head>` во все четыре страницы. Исходный
сниппет лежит в `../conwix/yandex.md` — он вне `public/`, наружу не отдаётся.
Меняется счётчик или его настройки — правятся все четыре страницы разом:
шаблонизатора здесь нет, и заводить его ради одного вендорского сниппета незачем.
